...
int farmer() {
 ...
 for ( i = 0 ; i < ROUNDS ; i ++ ) {
	 ...
	 if ( write(cg[1],&a,sizeof(a)) < 0 || write(gw[1],&a,sizeof(a)) < 0 ) { 
			perror(NULL) ; 
			return 1 ; 
	}  //when farmer enters, gives each pipe an "extra" slot, allowing both to enter if desired
	 ...
	 if ( read(cg[0],&a,sizeof(a)) < 0 || read(gw[0],&a,sizeof(a)) < 0 ) { 
			perror(NULL) ; 
			return 1 ; 
	} //the farmer takes away the "extra" slots, but must wait for such a slot to be free when one of the two leaves
...
int cabbage() {
 ...
 for ( i = 0 ; i < ROUNDS ; i ++ ) {
	 ...
	 if ( read(cg[0],&a,sizeof(a)) < 0 ) { 
			perror(NULL) ; 
			return 1 ; 
	} //only one of the pair can be in at once
...
if (close(cg[0])<0 || close(cg[1])<0 || close(gw[0])<0 || close(gw[1])<0) 
